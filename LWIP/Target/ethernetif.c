...
static void ethernetif_clean_dcache_by_addr(uint32_t address, uint32_t length)
{
  ...
  addr = address & ~(CACHE_LINE_SIZE - 1U);
  len = length + (address - addr);
  len = (len + CACHE_LINE_SIZE - 1U) & ~(CACHE_LINE_SIZE - 1U);

  SCB_CleanDCache_by_Addr((uint32_t *)addr, (int32_t)len);
}

static void ethernetif_invalidate_dcache_by_addr(uint32_t address, uint32_t length)
{
  uint32_t addr;
  uint32_t len;
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  for(q = p; q != NULL; q = q->next)
  {
    ...
    ethernetif_clean_dcache_by_addr((uint32_t)q->payload, q->len);
...
void ethernet_link_check_state(struct netif *netif)
{
  ...
  PHYLinkState = LAN8742_GetLinkState(&LAN8742);
	/* LAN8720 兼容处理：BSR 已经确认链路完成时，绕过 LAN8742 扩展状态误判。 */
