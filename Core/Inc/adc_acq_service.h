#ifndef __ADC_ACQ_SERVICE_H
#define __ADC_ACQ_SERVICE_H

#include "main.h"

#define ADC_ACQ_CHANNEL_COUNT 12U

/*
 * DMA double buffer model:
 *   one half-buffer contains this many complete sample groups.
 *   one sample group contains all 12 logical ADC channels.
 */
#define ADC_ACQ_GROUPS_PER_HALF 256U

typedef struct
{
    /* Current code uses this as a monotonically increasing sample sequence. */
    uint32_t timestamp_us;
    uint16_t raw[ADC_ACQ_CHANNEL_COUNT];
} adc_acq_sample_t;

typedef struct
{
    uint32_t adc1_half_count;
    uint32_t adc2_half_count;
    uint32_t adc3_half_count;

    uint32_t adc1_full_count;
    uint32_t adc2_full_count;
    uint32_t adc3_full_count;

    uint32_t sample_seq;
    uint32_t no_block_count;
} adc_acq_stats_t;

void adc_acq_service_init(void);
void adc_acq_service_start(void);
uint8_t adc_acq_service_get_sample(adc_acq_sample_t *sample);
void adc_acq_service_get_stats(adc_acq_stats_t *stats);

#endif /* __ADC_ACQ_SERVICE_H */
